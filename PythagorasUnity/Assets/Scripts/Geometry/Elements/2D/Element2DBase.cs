...
namespace RJWard.Geometry
{
	abstract public class Element2DBase : ElementBase
	{
		...
		{
			throw new System.InvalidOperationException( "Shouldn't be calling this, derived classes must use Element2DBase.Init( GeometryFactory gf, Field f, float d, int n ) so as to set numVertices" );
		}
		...
		override public T Clone<T>( string name, float d )
		{
			...
			Element2DBase e = clone as Element2DBase;
			if (e == null)
			{
				throw new System.Exception( "Cloning " + name + " doesn';t giuve an Element2DBase" );
			}
